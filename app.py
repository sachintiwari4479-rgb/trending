...
def fetch_meesho_data(query, limit=10, page=1, offset=0, custom_cookie=""):
    ...
    # Add cookie if provided by the user to bypass 403 Forbidden errors
    if custom_cookie:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error("API Error 403: Forbidden. Meesho blocked the request. Please provide a fresh Cookie in the sidebar.")
            return None
        else:
...
with st.sidebar:
    ...
    st.warning("To prevent 403 Forbidden errors, paste a fresh Cookie from your browser.")
    cookie_input = st.text_input("Meesho Cookie (Optional but recommended)", type="password", help="Go to Meesho -> F12 (Network) -> Search something -> Click 'search' request -> Copy 'Cookie' from Request Headers.")
    
    st.divider()
    
    st.header("Search Parameters")
    auto_category = st.selectbox("Auto-Discover Categories", ["Custom Search...", "Smartwatches", "Home Decor", "Sarees", "Kitchen Gadgets", "Makeup", "Toys", "Mens T-shirts"])
    ...
    fetch_limit = st.slider("Products Per Batch (Fetch Size)", min_value=10, max_value=500, value=50, step=10)
    ...
    st.markdown("We combine High Velocity, High Ratings, and Low RTO Risk to automatically rank the absolute best products.")
...
if st.button(f"🔍 Analyze '{search_query}' Trends", type="primary"):
    ...
    with st.spinner(f"Fetching first {fetch_limit} products and calculating Scores..."):
        raw_data = fetch_meesho_data(search_query, fetch_limit, 1, 0, cookie_input)
    ...
    with tab_main:
        if not df_genuine.empty:
            ...
            with tc2:
                ...
                st.markdown(f"This product was automatically selected as the best opportunity because it balances high daily sales velocity (**{best_product['Velocity (Ratings/Day)']} ratings/day**) with a strong average rating (**{best_product['Avg Rating']}⭐**) and manageable return risks.")
            ...
            st.warning("No genuine product trends found in this search. Try fetching more products or adjusting the velocity filter.")

    # TAB 2: TOP SUPPLIERS (SUPPLIER METRICS)
    with tab_suppliers:
        if not df_supplier.empty:
            st.info("💡 **NOTE:** These are completely new or unrated products. The huge velocity and ratings shown below belong to the **Supplier's entire account**, not the specific item. Use these to manually spot new uploads from successful sellers!")
    ...
    with col2:
        if st.button("⬇️ Fetch More Products (Load Next Page)", use_container_width=True, type="primary"):
            ...
            with st.spinner(f"Fetching next {fetch_limit} products..."):
                raw_data = fetch_meesho_data(st.session_state.current_query, fetch_limit, st.session_state.page, st.session_state.offset, cookie_input)
                if raw_data:
                    parsed_data = process_catalogs(raw_data)
                    if parsed_data:
                        st.session_state.all_products.extend(parsed_data)
